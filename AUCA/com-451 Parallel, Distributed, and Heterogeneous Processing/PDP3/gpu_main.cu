...
/******************************************************************************/
int updatePalette(GPU_Palette* P, Points* Points)
{
  for (int i = 0; i < NUMBER_OF_POINTS; ++i) {
  // for (Point Point : Points.points) {

    updateReds   <<< P->gBlocks, P->gThreads >>> (P->red,   Points->points[i]);
    updateGreens <<< P->gBlocks, P->gThreads >>> (P->green, Points->points[i]);
    updateBlues  <<< P->gBlocks, P->gThreads >>> (P->blue,  Points->points[i]);
  }
  return 0;
}
