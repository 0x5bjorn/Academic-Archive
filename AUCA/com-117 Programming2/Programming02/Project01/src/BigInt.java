	...
	public BigInt add(BigInt other) throws Exception {
		if (sign>0 && other.sign<0) {
			if (comparison(other) == 0)
				...
				return new BigInt(substraction(other).toString(), comparison(other) == 1 ? 1 : -1);
		...
		return new BigInt(addition(other).toString(), sign<0 && other.sign<0 ? -1 : 1);
	}
	
	public BigInt substract(BigInt other) throws Exception {
		if (sign>0 && other.sign<0) {
			return new BigInt(addition(other).toString(), 1);
		} else if (sign<0 && other.sign>0) {
			return new BigInt(addition(other).toString(), -1);
		} else if (sign<0 && other.sign<0) {
			if (comparison(other) == 0)
